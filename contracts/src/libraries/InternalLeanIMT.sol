...
struct LeanIMTData {
    // The poseidon2 implementation to use.
...
library InternalLeanIMT {
    ...
    function _insert(LeanIMTData storage self, uint256 leaf) internal returns (uint256) {
        ...
        for (uint256 level = 0; level < treeDepth; ) {
            if ((index >> level) & 1 == 1) {
                node = self.poseidon2.hash_2(self.sideNodes[level], node);
            } else {
    ...
    function _update(
        ...
        for (uint256 level = 0; level < treeDepth; ) {
            if ((index >> level) & 1 == 1) {
                ...
                node = self.poseidon2.hash_2(siblingNodes[i], node);
                oldRoot = self.poseidon2.hash_2(siblingNodes[i], oldRoot);
                ...
                if (index >> level != lastIndex >> level) {
                    ...
                    node = self.poseidon2.hash_2(node, siblingNodes[i]);
                    oldRoot = self.poseidon2.hash_2(oldRoot, siblingNodes[i]);
