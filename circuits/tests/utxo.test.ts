...
import { TransferType, type InputNote, type OutputNote } from "../src/types";
import { BN254_PRIME } from "../src/constants";
import { hashMessage, hexToBytes, parseSignature, recoverPublicKey, toHex } from "viem";
...
describe("utxo", () => {
  ...
  it("should get 2x2 proof without wormhole", async () => {
    ...
    console.log("publicKey", publicKey, hexToBytes(publicKey).length)

    console.log("account.publicKey", account.publicKey, hexToBytes(account.publicKey).length)
